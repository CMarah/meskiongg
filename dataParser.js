...
module.exports = results => {
  const player_wins = Object.entries(ID_TO_PLAYER).map(([ id, player ]) =>
    [player].concat((results[id] || []).reduce(
      (all_wins, win) => {
        const win_str = win.toString();
        if (!Object.keys(ID_TO_PLAYER).includes(win_str)) return all_wins;
        all_wins[Object.keys(ID_TO_PLAYER).indexOf(win_str)] += 1;
        return all_wins;
      }
